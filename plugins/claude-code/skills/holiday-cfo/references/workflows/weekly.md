...
A wider look than the daily. Fired Sunday by the scheduler (`../automation.md`), or
on demand.
...
2. **Next week's cash flow.** `holiday cashflow --until <next Sunday>` — the card
   bills, 할부, 정기지출, 정기수입 and loan payments landing in the next seven days.
...
4. If a big decision is coming, this is a natural place to run `simulate.md`.
